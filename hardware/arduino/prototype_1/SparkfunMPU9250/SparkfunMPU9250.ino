...
volatile bool flag, timerFlag;
unsigned int t, timeVar;
int tmax;
int filenum;
int SRate;
String f1, f2, filename;
...
void setup()
{
  ...
  pinMode(53, OUTPUT);
  digitalWrite(53, HIGH); // davekw7x: If it's low, the Wiznet chip corrupts the SPI bus
  ...
  timeVar=0;
  //SRate = 11;  //in ms
  //Sampling Rate is 30ms
  tmax = 150;    //number of samples
  flag = false;
  timerFlag = false;
  filenum = 0;
  f1 = "Test";
...
void loop()
{
  ...
  MahonyQuaternionUpdate(myIMU.ax, myIMU.ay, myIMU.az, myIMU.gx * DEG_TO_RAD,
    ...
    //if (myIMU.delt_t > SRate)
    //{
       filename = f1 + filenum + f2;
       File testFile = SD.open(filename, FILE_WRITE);
     
      if (testFile) {
       timeVar += 30;
        String dataString = "";
        dataString += String(timeVar);
        dataString += ", ";
        ...
        dataString += ", ";
        dataString += String((int)1000 * myIMU.gx);
        dataString += ", ";
        dataString += String((int)1000 * myIMU.gy);
        dataString += ", ";
        dataString += String((int)1000 * myIMU.gz);
        dataString += ", ";
