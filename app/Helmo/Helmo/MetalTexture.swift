...
class MetalTexture: NSObject {
  ...
  func loadTexture(device: MTLDevice, commandQ: MTLCommandQueue, flip: Bool) {
    ...
    let texDescriptor = MTLTextureDescriptor.texture2DDescriptor(pixelFormat: MTLPixelFormat.rgba8Unorm, width: Int(width), height: Int(height), mipmapped: isMipmapped)
  ...
  class func textureCopy(source: MTLTexture, device: MTLDevice, mipmapped: Bool) -> MTLTexture {
    ...
    let texDescriptor = MTLTextureDescriptor.texture2DDescriptor(pixelFormat: MTLPixelFormat.bgra8Unorm, width: Int(source.width), height: Int(source.height), mipmapped: mipmapped)
  ...
  class func copyMipLayer(source: MTLTexture, destination: MTLTexture, mipLvl: Int) {
    ...
    let mipmappedWidth = max(Int(source.width)/q,1)
    let mipmappedHeight = max(Int(source.height)/q,1)
    
    let region = MTLRegionMake2D(0, 0, mipmappedWidth, mipmappedHeight)
    let pixelsData = malloc(mipmappedHeight * mipmappedWidth * 4)
    source.getBytes(pixelsData!, bytesPerRow: mipmappedWidth * 4, from: region, mipmapLevel: mipLvl)
    destination.replace(region: region, mipmapLevel: mipLvl, withBytes: pixelsData!, bytesPerRow: mipmappedWidth * 4)
    free(pixelsData)
  }
  ...
  func image(mipLevel: Int) -> UIImage {
    ...
    let mipmappedWidth = max(width / q,1)
    let mipmappedHeight = max(height / q,1)
    let rowBytes = mipmappedWidth * 4
    
    let colorSpace = CGColorSpaceCreateDeviceRGB()
    
    let context = CGContext(data: p, width: mipmappedWidth, height: mipmappedHeight, bitsPerComponent: 8, bytesPerRow: rowBytes, space: colorSpace, bitmapInfo: CGImageAlphaInfo.premultipliedLast.rawValue)
  ...
  func bytesForMipLevel(mipLevel: Int) -> UnsafeMutableRawPointer {
    ...
    let q = Int(powf(2, Float(mipLevel)))
    let mipmappedWidth = max(Int(width) / q,1)
    let mipmappedHeight = max(Int(height) / q,1)
    
    let rowBytes = Int(mipmappedWidth * 4)
    
    let region = MTLRegionMake2D(0, 0, mipmappedWidth, mipmappedHeight)
    let pointer = malloc(rowBytes * mipmappedHeight)
